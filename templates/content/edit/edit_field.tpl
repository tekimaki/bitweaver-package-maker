...
{{if !empty($field.input.type)}}
    ...
    {{elseif $field.input.type=="radio"}}
		<ul>
		{{foreach from=$field.validator.choices item=choice}}
		<li><label for="{{$namespace}}[{{$fieldName}}]"><input type="radio" name="{{$namespace}}[{{$fieldName}}]" id="{{$fieldId}}" value="{{$choice}}" {if $gContent->getField("{{$fieldName}}")}checked="checked"{/if}/>&nbsp;{{$choice}}</label></li>
		{{/foreach}}
		</ul>
    {{elseif $field.input.type=="checkbox"}}
		<ul>
		{{foreach from=$field.validator.choices item=choice}}
		<li><label for="{{$namespace}}[{{$fieldName}}]"><input type="checkbox" id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" value="{{$choice}}" {if $gContent->getField("{{$fieldName}}")}checked="checked"{/if}/>&nbsp;{{$choice}}</label></li>
		{{/foreach}}
		</ul>
    {{elseif $field.input.type=="hexcolor"}}
    	    <input class="textInput" type="text" class="color {ldelim}required:{{if $field.validator.required}}true{{else}}false{{/if}}{rdelim}" id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" value="{$gContent->getField("{{$fieldName}}")}" />
    {{elseif $field.input.type=="hexcolor"}}
    	    <input class="textInput" type="text" class="color {ldelim}required:{{if $field.validator.required}}true{{else}}false{{/if}}{rdelim}" id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" value="{$gContent->getField("{{$fieldName}}")}" />
    ...
    {{elseif $field.input.type=="boolean"}}
    	    <input type="checkbox" id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" {if $gContent->getField("{{$fieldName}}")}checked="checked"{/if}/>
    {{elseif $field.input.type=="int" || $field.input.type=="long"}}
    	    <input class="textInput" type="text" id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" value="{$gContent->getField("{{$fieldName}}")}" />
    ...
    {{elseif $field.input.type=="textarea"}}
	    	<textarea id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" rows="{{$field.input.rows|default:"20"}}">{$gContent->getField("{{$fieldName}}")}</textarea>
    {{elseif $field.input.type=="parsed"}}
            {textarea label="{{$field.name}}" error=$errors.{{$fieldName}} help="{{$field.help}}" noformat="true" id="{{$fieldId}}" name="{{$namespace}}[{{$fieldName}}]" rows="{{$field.input.rows|default:"20"}}"}{$gContent->getField("{{$fieldName}}")}{/textarea}
