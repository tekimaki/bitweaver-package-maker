...
{{foreach from=$typemap.attachments key=attachment item=prefs}}
			...
			if( !empty( $_FILES['{{$typemapName}}_{{$attachment}}']['tmp_name'] ) ){
				...
				if( $this->mServiceContent->storeAttachment( $fileStoreHash ) ){
					...
					$old_{{$attachment}}_id = $this->mDb->getAssoc("SELECT `content_id`, `{{$attachment}}_id` FROM `{{$type.name}}_{{$typemapName}}` WHERE `content_id` = ?", array('content_id' => $pParamHash['{{$type.name}}']['{{$typemapName}}']['content_id']));
				}
			}
{{/foreach}}
