...
{{if $typemap.services && in_array('content_edit_mini',$typemap.services.templates)}}
{{if !$typemap.service_prefs.edit_loc || in_array('content',$typemap.service_prefs.edit_loc)}}
...
{/if}
{{else}}
{if $smarty.request.preflight_fieldset_guid eq '{{$typemapName}}'}
{include file="bitpackage:{{$config.package}}/{{$config.plugin}}/edit_{{$typemapName}}_inc.tpl"}
{/if}
{{/if}}
{{/if}}
